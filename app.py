...
import json # Necessário para converter a lista do carrinho em texto para a planilha
...
def carregar_historico_orc():
    try:
        ...
        if df is not None:
            ...
            # Garante que a coluna de dados técnicos exista
...
def secao_orcamento(df_ing, margem_lucro, taxa_credito_input, forma_pagamento):
    # --- LÓGICA DE CARREGAMENTO (RESOLVE O ERRO DA IMAGEM) ---
        ...
        if st.session_state.carrinho_orc:
            ...
            if b_col2.button("💾 Salvar Orçamento", use_container_width=True):
                ...
                if df_hist is not None:
                    # Salva os itens em formato JSON para poder editar depois
