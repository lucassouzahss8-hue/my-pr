...
# --- FUNÇÕES DE CARREGAMENTO COM CACHE (RESOLVE O ATRASO) ---
...
if "n_itens_receita" not in st.session_state: st.session_state.n_itens_receita = 1
    ...
    with t1:
        ...
        if st.session_state.carrinho_orc:
            ...
            for idx, it in enumerate(st.session_state.carrinho_orc):
                ...
                lista_pdf.append({"nome": it['nome'], "qtd": it['qtd'], "venda": v_venda_it * (1 + taxa_perc)})
                c[0].write(it['nome']); c[1].write(f"x{it['qtd']}"); c[2].write(f"R$ {it['preco_puro']:.2f}")
                c[3].write(f"R$ {v_unit_custo_exibicao:.2f}"); c[4].write(f"**R$ {v_venda_it:.2f}**")
            ...
            v_lucro_orc = total_venda_bruta_acumulada - (total_ingredientes_acumulados + emb_val)
            
            cmv_perc_orc = ((total_ingredientes_acumulados + emb_val) / total_venda_bruta_acumulada * 100) if total_venda_bruta_acumulada > 0 else 0
            ...
            with res2:
                st.markdown(f"<div class='resultado-box'><p style='margin:0; font-size:13px; opacity:0.8;'>RESUMO</p><p><b>CMV:</b> <span style='color:{cor_cmv_orc};'>{cmv_perc_orc:.1f}%</span></p><p><b>Lucro:</b> R$ {v_lucro_orc:.2f}</p><h2 style='margin:0;'>R$ {total_geral_orc:.2f}</h2></div>", unsafe_allow_html=True)
            ...
            if b_col2.button("💾 Salvar Orçamento", use_container_width=True):
                ...
                novo_reg = pd.DataFrame([{"Data": data_orc.strftime("%d/%m/%Y"), "Cliente": nome_cliente, "Pedido": nome_grupo_pedido, "Valor_Final": f"R$ {total_geral_orc:.2f}", "Itens_JSON": json.dumps(st.session_state.carrinho_orc)}])
            ...
            if b_col3.button("🗑️ Limpar Pedido", use_container_width=True):
                st.session_state.carrinho_orc = []; st.rerun()
    ...
    with t2:
        ...
        if not df_salvos.empty:
            for i, row in df_salvos.iterrows():
                ...
                c1.write(row.get('Data')); c2.write(row.get('Cliente')); c3.write(row.get('Pedido')); c4.write(row.get('Valor_Final'))
                if c5.button("📝", key=f"ed_h_{i}"):
                    st.session_state.carregar_orc_dados = {"cliente": row.get('Cliente'), "pedido": row.get('Pedido'), "itens": json.loads(row.get('Itens_JSON', '[]'))}
                    st.rerun()
...
def main():
    ...
    with st.expander("📂 Abrir ou Deletar Receitas Salvas"):
        ...
        with col_rec1: receita_selecionada = st.selectbox("Selecione uma receita:", [""] + receitas_nomes)
        with col_rec2:
            st.write("") 
    ...
    nome_produto_final = col_p1.text_input("Nome do Produto Final:", key="nome_prod_input")
    margem_lucro = col_p2.number_input("Margem de Lucro (%)", min_value=0, value=135)
    distancia_km = col_p3.number_input("Distância (km)", min_value=0.0, value=0.0)
    forma_pagamento = col_p4.selectbox("Pagamento", ["Crédito", "PIX"])
    ...
    with col_esq:
        ...
        for i in range(st.session_state.n_itens_receita):
            ...
            escolha = c1.selectbox(f"Item {i+1}", options=lista_nomes, index=idx_nome, key=f"nome_{i}_{st.session_state.versao_lista}")
            st.session_state[f"nome_{i}"] = escolha
            dados_item = df_ing[df_ing['nome'] == escolha].iloc[0]
            val_qtd = st.session_state.get(f"qtd_{i}", 0.0)
            qtd_usada = c2.number_input(f"Qtd", value=val_qtd, key=f"qtd_{i}_{st.session_state.versao_lista}", step=0.01)
            ...
            unid_uso = c3.selectbox(f"Unid", options=unid_opcoes, index=idx_unid, key=f"u_{i}_{st.session_state.versao_lista}")
            st.session_state[f"u_{i}"] = unid_uso
            fator = 0.001 if (unid_uso == "g" and dados_item['unidade'] == "kg") or (unid_uso == "ml" and dados_item['unidade'] == "l") else 1.0
            custo_parcial = (qtd_usada * fator) * float(dados_item['preco'])
            custo_ingredientes_total += custo_parcial
            lista_para_salvar.append({"nome_rece_ita": nome_produto_final, "ingrediente": escolha, "qtd": qtd_usada, "unid": unid_uso})
            c4.markdown(f"<p style='padding-top:35px; font-weight:bold;'>R$ {custo_parcial:.2f}</p>", unsafe_allow_html=True)
            if c5.button("❌", key=f"del_i_main_{i}"): st.session_state.n_itens_receita -= 1; st.rerun()
    ...
    t_perc = (taxa_credito_input / 100) if forma_pagamento == "Crédito" else 0.0
    v_taxa_fin = (preco_venda_produto + taxa_entrega) * t_perc
    preco_venda_final = preco_venda_produto + taxa_entrega + v_taxa_fin
    cmv_perc = (v_cmv / preco_venda_produto * 100) if preco_venda_produto > 0 else 0
    cor_cmv = "#4ade80" if cmv_perc <= 35 else "#facc15" if cmv_perc <= 45 else "#f87171"
    ...
    with res1:
        ...
        st.table(pd.DataFrame({
            ...
            "Valor": [f"R$ {custo_ingredientes_total:.2f}", f"R$ {v_quebra:.2f}", f"R$ {v_despesas:.2f}", f"R$ {valor_embalagem_manual:.2f}", f"R$ {custo_total_prod:.2f}", f"{cmv_perc:.1f}%", f"R$ {lucro_valor:.2f}", f"R$ {taxa_entrega:.2f}", f"R$ {v_taxa_fin:.2f}", f"R$ {preco_venda_final:.2f}"]
        }))
        if st.button("💾 Salvar Receita", use_container_width=True) and nome_produto_final:
    ...
    with res2:
        st.markdown(f"<div class='resultado-box'><p>VALOR SUGERIDO</p><h1 style='font-size:48px;'>R$ {preco_venda_final:.2f}</h1><hr><p><b>Lucro:</b> <span style='color:#4ade80;'>R$ {lucro_valor:.2f}</span></p><p><b>CMV:</b> <span style='color:{cor_cmv};'>{cmv_perc:.1f}%</span></p></div>", unsafe_allow_html=True)
