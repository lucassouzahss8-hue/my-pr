...
def main():
    ...
    with col_esq:
        ...
        n_itens_input = st.number_input("Número de itens:", min_value=1, key="n_itens_val")
    ...
    # --- GERADOR DE ORÇAMENTOS (ADICIONADO) ---
    st.divider()
    st.markdown("<h2 class='titulo-planilha'>📋 Gerador de Orçamentos</h2>", unsafe_allow_html=True)
    
    col_orc1, col_orc2, col_orc3 = st.columns([3, 1, 1])
    with col_orc1:
        item_orc = st.selectbox("Adicionar ao Orçamento:", options=[""] + df_ing['nome'].tolist(), key="sel_orc")
    with col_orc2:
        qtd_orc = st.number_input("Quantidade", min_value=1, value=1, key="qtd_orc")
    with col_orc3:
        st.write("")
        if st.button("➕ Adicionar", use_container_width=True):
            if item_orc:
                # Busca custo puro na planilha
                custo_puro = float(df_ing[df_ing['nome'] == item_orc]['preco'].iloc[0])
                st.session_state.carrinho_orc.append({
                    "item": item_orc,
                    "qtd": qtd_orc,
                    "custo_puro": custo_puro
                })
                st.rerun()

    if st.session_state.carrinho_orc:
        total_custo_fabrica_geral = 0.0
        total_venda_orc = 0.0
        ...
        cols_h[3].write("**Unit. Venda**")
        cols_h[4].write("**Subtotal**")

        for idx, it in enumerate(st.session_state.carrinho_orc):
            c = st.columns([3, 1, 1.5, 1.5, 2, 0.5])
            
            # Cálculos de Venda (Custo Puro + Margem da tela)
            venda_unitario = it['custo_puro'] * (1 + (margem_lucro / 100))
            subtotal_venda = venda_unitario * it['qtd']
            
            total_custo_fabrica_geral += (it['custo_puro'] * it['qtd'])
            total_venda_orc += subtotal_venda

            c[0].write(it['item'])
            c[1].write(f"x{it['qtd']}")
            c[2].write(f"R$ {it['custo_puro']:.2f}") # UNITÁRIO PURO
            c[3].write(f"R$ {venda_unitario:.2f}")    # UNITÁRIO VENDA
            c[4].write(f"**R$ {subtotal_venda:.2f}**")
        ...
        with res_o1:
            st.write(f"💵 Total Custo de Fábrica: **R$ {total_custo_fabrica_geral:.2f}**")
            st.write(f"🚚 Entrega: R$ {taxa_entrega:.2f}")
        with res_o2:
            # Cálculo de taxa financeira final
            base_final_orc = total_venda_orc + taxa_entrega
            v_taxa_orc = base_final_orc * (taxa_credito_input / 100) if forma_pagamento == "Crédito" else 0.0
            
            if forma_pagamento == "Crédito":
                st.write(f"💳 Taxa Cartão: R$ {v_taxa_orc:.2f}")
            st.markdown(f"### TOTAL ORÇAMENTO: R$ {base_final_orc + v_taxa_orc:.2f}")
