...
# 2. Estilização CSS (Sua base visual)
st.markdown("""
    <style>
...
# --- FUNÇÕES DE CARREGAMENTO (Sua base de dados) ---
...
def main():
    ...
    st.markdown("<h1 class='titulo-planilha'>📊 Precificador de Trufas</h1>", unsafe_allow_html=True)
    ...
    # --- ÁREA DOS INGREDIENTES (Precificador de Fábrica) ---
    ...
    # --- DETALHAMENTO DO PRECO ---
    st.divider()
    res1, res2 = st.columns([1.5, 1])
    with res1:
        st.markdown(f"### Detalhamento: {nome_produto_final if nome_produto_final else 'Produto'}")
    ...
    # --- GERADOR DE ORÇAMENTOS (ATUALIZADO) ---
    st.divider()
    ...
    with tab_gerar:
        ...
        nome_produto_grupo = st.text_input("📌 Nome do Pedido", placeholder="Ex: Caixa Presente 12 Trufas")

        st.divider()
        co1, co2, co3, co4 = st.columns([3, 1, 1, 1])
        item_sel = co1.selectbox("Selecione o Item:", options=[""] + df_ing['nome'].tolist(), key="sel_item_orc")
        ...
        if st.button("➕ Adicionar ao Pedido"):
            if item_sel != "":
                ...
                if not filtro.empty:
                    ...
                    # Salva custo puro para mostrar sem acréscimos depois
                    st.session_state.carrinho_orc.append({
                        "Item": item_sel, 
                        "Qtd": qtd_orc, 
                        "Custo_Puro": p_base_cru,
                        "Frete": f_orc,
                        "Emb": e_orc
                    })
                    st.rerun()
        ...
        if st.session_state.carrinho_orc:
            ...
            total_custos_sem_acrescimo = 0.0
            ...
            h[1].write("**Qtd**")
            h[2].write("**Unit. Puro**")
            h[3].write("**Unit. Venda**")
            h[4].write("**Subtotal Venda**")
            ...
            for idx, it in enumerate(st.session_state.carrinho_orc):
                ...
                # Cálculo da Venda (Custo + Adicionais + Lucro)
                # Respeita os parâmetros de Quebra e Despesas do precificador
                custo_prod = c_puro + (c_puro * (perc_quebra/100)) + (c_puro * (perc_despesas/100)) + it['Frete'] + it['Emb']
                venda_unitario = custo_prod * (1 + (margem_lucro_input / 100))
                sub_venda = venda_unitario * q
                
                total_custos_sem_acrescimo += (c_puro * q)
                total_venda_acumulado += sub_venda
                
                cols[0].write(f"🔹 {it['Item']}")
                cols[1].write(f"x{q}")
                cols[2].write(f"R$ {c_puro:.2f}") # VALOR SEM ACRÉSCIMO
                cols[3].write(f"R$ {venda_unitario:.2f}") # VALOR COM LUCRO
                cols[4].write(f"**R$ {sub_venda:.2f}**")
            ...
            # --- TOTAIS DO ORÇAMENTO ---
            # Soma a entrega por distância (sidebar) ao total de venda
            ...
            # Aplica taxa de cartão se selecionado
            ...
            res_o1, res_o2 = st.columns(2)
            with res_o1:
                st.markdown("#### 🛠️ Resumo Interno")
                st.write(f"Total Custos (Preço de Fábrica): **R$ {total_custos_sem_acrescimo:.2f}**")
                st.write(f"Lucro Bruto Previsto: R$ {(total_venda_acumulado - total_custos_sem_acrescimo):.2f}")
            
            with res_o2:
                st.markdown("#### 💰 Total do Orçamento")
                st.write(f"Subtotal Venda: R$ {total_venda_acumulado:.2f}")
                if v_entrega_final > 0: st.write(f"Entrega: R$ {v_entrega_final:.2f}")
                if forma_pagamento_input == "Crédito": st.write(f"Taxa Financeira ({taxa_credito_input}%): R$ {v_taxa_cartao:.2f}")
                st.markdown(f"<h2 style='color: #4ade80;'>Total: R$ {total_geral_orc:.2f}</h2>", unsafe_allow_html=True)

            # Botões
            b1, b2, b3 = st.columns(3)
            if b1.button("📱 Texto WhatsApp", use_container_width=True):
                msg = f"✨ *ORÇAMENTO* ✨\n\n👤 *Cliente:* {nome_cliente}\n🎁 *Pedido:* {nome_produto_grupo}\n\n"
                for i in st.session_state.carrinho_orc:
                    msg += f"✅ {i['Item']} (x{i['Qtd']})\n"
                msg += f"\n💰 *Total:* R$ {total_geral_orc:.2f}\n💳 *Pagamento:* {forma_pagamento_input}\n📍 *Entrega:* R$ {v_entrega_final:.2f}"
                st.code(msg, language="text")
            
            if b2.button("💾 Salvar Histórico", use_container_width=True):
